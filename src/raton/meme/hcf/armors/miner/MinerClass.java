...
public class MinerClass extends PvpClass implements Listener {
    ...
    public void onMinerSpeedClick(PlayerInteractEvent event) {
        ...
        if (((action == Action.RIGHT_CLICK_AIR) || (action == Action.RIGHT_CLICK_BLOCK)) &&
            ...
            long timestamp = minerSpeedCooldowns.get(uuid);
            long millis = System.currentTimeMillis();
            long remaining = timestamp == minerSpeedCooldowns.getNoEntryValue() ? -1L : timestamp - millis;
            if (remaining > 0L) {
                ...
                minerSpeedCooldowns.put(event.getPlayer().getUniqueId(), System.currentTimeMillis() + MINER_SPEED_COOLDOWN_DELAY);
            }
        }
    }
