...
public class ScoreboardHelper implements SidebarProvider {
    ...
    public List<SidebarEntry> getLines(Player player) {
        ...
        if (SettingsYML.KIT_MAP) {
            ...
            lines.add(new SidebarEntry(ChatColor.RED.toString() + "Kills: §f" + player.getStatistic(Statistic.PLAYER_KILLS)));
            lines.add(new SidebarEntry(ChatColor.RED.toString() + "Deaths: §f" + player.getStatistic(Statistic.DEATHS)));
        }
