...
import raton.meme.hcf.factionutils.event.CaptureZoneEnterEvent;
import raton.meme.hcf.factionutils.event.CaptureZoneLeaveEvent;
...
public class ProtectionListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (HCF.getPlugin().getFactionManager().getFactionAt(event.getPlayer().getLocation()).getName().equalsIgnoreCase("Warzone") && event.getAction() == Action.PHYSICAL && (event.getClickedBlock().getType().equals(Material.STONE_PLATE) || event.getClickedBlock().getType().equals(Material.WOOD_PLATE))) {
            return;
        }
